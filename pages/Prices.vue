    ...
    <NuxtLayout name="home">
      <section class="md:pb-25">
        <div>
          ...
          <div class="grid <md:grid-cols-2 md:grid-cols-3 text-white md:gap-10 <md:gap-4">
            <div v-if="tokens" v-for="(token,index) in tokens.filter(t=>t.price)">
              <div class="flex flex-col justify-center items-center bg-primary-600 border-5 border-stake-100 rounded-md pb-10 min-h-full shadow-sm shadow-stake-100">
                <MyChart :data="data"/>
                <div v-if="!token.image || token.name == 'Frax'" class="font-brandon uppercase rounded-circle bg-white text-primary-500 w-14 h-14 flex justify-center align-center">{{token.symbol.slice(0,1)}}</div>
                <img :alt="`image-${token.network}`" class="w-14" v-if="token.image && token.name != 'Frax'" :src="token.image"/>
                <div class="font-brandonlight font-bold text-center text-3xl my-4">{{ token.name }}</div>
        ...
        <div>
          ...
          <div class="grid <md:grid-cols-2 md:grid-cols-4  text-white md:gap-10 <md:gap-4">
            <div v-if="tokens" v-for="(token,index) in tokens.filter(t=>!t.price)">
              <div class="flex flex-col justify-center items-center bg-primary-600 border-5 border-stake-100 rounded-md pa-6 min-h-full shadow-sm shadow-stake-100">
                <img :alt="`image-${token.name}`" class="w-14" v-if="token.image" :src="token.image"/>
